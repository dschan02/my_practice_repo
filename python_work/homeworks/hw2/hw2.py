...
def middle_three(str):
    ...
    """
    #checks if string is less than 3 or even
    if len(str) < 3 or (len(str) % 2 == 0):
        return ""
    
    #string is valid length, then below
    middle_index = len(str) // 2    # gives the number of the middle index
    return str[middle_index-1] + str[middle_index] + str[middle_index+1]
...
def filter_long_words(words, length):
    ...
    """
    return [word for word in words if len(word) > length]   #returns list made with list compreh. if length of string is greater then input length
...
def unique_characters(word):
    ...
    """
    return set([char for char in word])   #indexes characters in string in a list and returns its set conversion 
...
def merge_dictionaries(dict1, dict2):
    ...
    """
    merged_dict = dict1
    for key, value in dict2.items():       #will overwrite dict1 shared key values to dict2 values
...
def character_frequency(strings):
    ...
    for word in strings:     #indexes each word in list of strings
        for char in word:       #indexes characters in word
...
def group_by_length(strings):
    ...
    for word in strings:
        key = len(word)
        if key not in len_dict:     #checks if key does not already exist
            len_dict[key] = []  
        len_dict[key].append(word)     #append to list
    return len_dict
